...
# This file was *autogenerated* from the file experiments/prediction/showcase.sage
from sage.all_cmdline import *   # import sage library

_sage_const_2 = Integer(2); _sage_const_10 = Integer(10); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_5 = Integer(5); _sage_const_1p0 = RealNumber('1.0')
...
def get_parser():
    ...
    parser.add_argument("--num_prints", type=int, default=_sage_const_10 )
...
def is_reduced_GB(G):
    ...
    for i, g1 in enumerate(G):
        for j, g2 in enumerate(G):
            if i == j: continue
            lm = g1.lm()
            if np.any([term.reduce([lm]) == _sage_const_0  for term in g2.monomials()]):
                return False 
...
def showcase(num_var, field, term_order, data_encoding, data_path, save_dir, verbose=True):
    ...
    for i, sample in enumerate(testset):
        ...
        stats = {'index': int(i),
                 ...
                 'linear_dimension': int(len(ideal(G).normal_basis())) if field != RR else _sage_const_0 ,  # error for RR
                 'size_F': int(len(F)),
    ...
    summary = {}
    for key in stats_list[_sage_const_0 ]:
    ...
    for key in summary:
        ...
        cc = np.corrcoef(summary['hit'], y=summary[key])[_sage_const_0 ,_sage_const_1 ]
        print(f'{key}: {cc:.3f}')
    print('-----------------------')    
...
def print_case(num_var, field, term_order, data_encoding, data_path, save_dir, case='incorrect', cmp_style='FG', verbose=True, num_prints=_sage_const_5 , latex_form=False):
    ...
    for k, (i, Ginc) in enumerate(case_Gs.items()):
        ...
        if latex_form:
            ...
            if cmp_style == 'FG':            
                ...
                for j in range(s):
                    ...
                    f = f'$f_{j+_sage_const_1 }$ = ${f}$' if f else f
                    g = f'& $g_{j+_sage_const_1 }$ = ${g}$ ' if g else g
                    if j == _sage_const_0 :
            ...
            if cmp_style == 'GG':
                ...
                for j in range(s):
                    ...
                    g = f'$g_{j+_sage_const_1 } = {g}$ ' if g else g
                    ginc = f"& $g\'_{j+_sage_const_1 } = {ginc}$ " if ginc else ginc
                    if j == _sage_const_0 :
        ...
        if num_prints > _sage_const_0  and k > num_prints: break 
    
    support_acc /= _sage_const_1p0  * tot
...
def check_reducity(num_var,field, term_order, save_dir, data_path, verbose=True):
    ...
    stats_list = []
    support_hit = _sage_const_0 
    GB_in_incorrect = _sage_const_0 
    reducedGB_in_incorrect = _sage_const_0 
    eqideal_in_incorrect = _sage_const_0 

    GB_in_correct = _sage_const_0 
    reducedGB_in_correct = _sage_const_0 
    eqideal_in_correct = _sage_const_0 
...
def main():
    ...
    if args.field  == 'RR':
        field = RR
    if args.field[_sage_const_0 ] == 'F' and args.field[_sage_const_1 :].isdigit():
        field = GF(int(args.field[_sage_const_1 :]))
