...
class Polynomial_Sampler():
    ...
    def sample(self, num_samples=1, size=None, density=1.0, matrix_type=None, max_iters=100):
        ...
        return_matrix = size is not None
        if not return_matrix:
            size = (num_samples, 1)
        
        num_terms = randint(1, max_num_terms) if max_num_terms else None 
        
        for i in range(max_iters):
            M = matrix.random(self.ring, *size, max_degree=max_degree, max_num_terms=num_terms, min_degree=min_degree, max_coeff=max_coeff, num_bound=num_bound)
            ...
            if density < 1.0:
                assert(return_matrix)
                M = [m if random() < density else m*0 for m in M.list()]
                M = matrix(self.ring, *size, M)
            
            if min([p.degree() for p in M.list()]) < min_degree: continue
            if nonzero_instance and all([p==0 for p in M.list()]): continue
            
            if i == max_iters-1:
                raise RuntimeError(f'Failed to find a nonzero polynomial with {max_iters} iterations')  
            
        if not return_matrix: M = M.list()
...
#             if coeff_ring == QQ:
#                 p = ring.random_element(degree=degree, terms=num_terms, num_bound=num_bound, choose_degree=False)
#             elif coeff_ring == RR:
#                 p = ring.random_element(degree=degree, terms=num_terms, min=-max_coeff, max=max_coeff, choose_degree=False)
#             elif coeff_ring == ZZ:
#                 p = ring.random_element(degree=degree, terms=num_terms, x=-max_coeff, y=max_coeff+1, choose_degree=False)
#             else:
#                 assert(coeff_ring.field().is_finite())
#                 p = ring.random_element(degree=degree, terms=num_terms, choose_degree=False)
